...
def Buscar_farmaco():
    ...
    while Continuar:
        ...
        #Los códigos ChEMBL tienen una estructura muy definida. Si la cadena introducida por el usuario no se ajusta a la misma, el programa lo indicará.s
...
def Busqueda_laxa_farmacos (Farmaco):
    ...
    if len(Datos) == 0 and len(Farmaco) >= 3:
        ...
        Datos=Cursor.fetchall()
        #Si se han encontrado resultados, se muestran al usuario para que pueda elegir.
        if len(Datos) != 0:
            print("No hemos encontrado el fármaco que busca. ¿Quizás se refería a alguno de los siguientes?:\n")
            Numero_inicial = 1
            #El uso del bucle permite la impresión limpia en pantalla de los resultados.
            for Fila in Datos:
            ...
            if re.search("[Ss][IiÍí]", Respuesta_usuario): 
                try:
                    Respuesta2_usuario = input("Por favor, seleccione el número de su fármaco de interés: ").replace("\t", "")
                    Farmaco = Datos[int(Respuesta2_usuario)-1][0]
        ...
        #Si no se encuentra ningún fármaco que incluya el patrón introducido, no es posible continuar con la consulta.
            ...
            print("El nombre del fármaco introducido no se encuentra en la base de datos. Por favor, inténtelo de nuevo.")
            Farmaco = None
...
#Esta función es análoga a "Busqueda_laxa_farmacos()", pero aplica para enfermedades.
#Para una explicación más detallada, consultar los comentarios de la función mencionada.
def Busqueda_laxa_enfermedades (Enfermedad): 
    Sentencia_estricta = "SELECT name FROM disease_code WHERE name = %s"
    Cursor.execute(Sentencia_estricta, (Enfermedad, ))
    Datos=Cursor.fetchall()
    #Si no aparecen resultados para la enfermedad introducida, pero tiene más de tres caracteres, buscar enfermedades con el patrón introducido.
    if len(Datos) == 0 and len(Enfermedad) >=3:
        Sentencia_laxa="""
                  SELECT name FROM disease_code 
                  WHERE name LIKE %s
                  ORDER BY name ASC;
                  """
        Enfermedad=f"%{Enfermedad}%"
        Cursor.execute(Sentencia_laxa, (Enfermedad,))
        Datos=Cursor.fetchall()
        #Si se han encontrado resultados, se muestran al usuario para que pueda elegir.
        if len(Datos) != 0:
            print("No hemos encontrado la enfermedad que busca. ¿Quizás se refería a alguna de las siguientes?:\n")
            Numero_inicial = 1
            #El uso del bucle permite la impresión limpia en pantalla de los resultados.
            for Fila in Datos:
                print(f"{Numero_inicial}\t{Fila[0]}")
                Numero_inicial+=1
            Respuesta_usuario = input(f"\n¿Se encuentra su enfermedad de interés en la lista? [En caso afirmativo, escriba \"Sí\"]: ").replace("\t", "")
            #Se pregunta al usuario si se encuentra en la lista proporcionada la enfermedad que le interesa.
            if re.search("[Ss][IiÍí]", Respuesta_usuario): 
                try:
                    Respuesta2_usuario = input("Por favor, seleccione el número de su enfermedad de interés: ").replace("\t", "")
                    Enfermedad = Datos[int(Respuesta2_usuario)-1][0]
                    print(f"Su elección ha sido: {Enfermedad}.\n") 
                except (ValueError, IndexError): 
                    print("\nPor favor, introduzca una opción válida.")
                    Enfermedad = None
            #Si el usuario no encuentra ninguna enfermedad de interés entre las opciones mostradas, no es posible continuar con la consulta.
            else: 
                print("Sentimos las molestias. Por favor, inténtelo de nuevo")
                Enfermedad = None
        #Si no se encuentra ninguna enfermedad que incluya el patrón introducido, no es posible continuar con la consulta.
        else:
            print("Lo sentimos, no se ha encontrado ninguna enfermedad que incluya ese término.")
            Enfermedad = None
    #Si la cadena introducida tiene menos de tres caracteres, no es posible continuar con la consulta.
    elif len(Datos) == 0 and len(Enfermedad) < 3: 
        print("El nombre de la enfermedad introducida no se encuentra en la base de datos. Por favor, inténtelo de nuevo.")
        Enfermedad = None
    return Enfermedad
...
def Coger_informacion_farmaco_especifico(Farmaco):
    #La variable "Farmaco" la llenará el usuario gracias a la función "Buscar_farmaco()" (se ve en "Funcionalidades_2()").
    Consulta_SQL = """
    ...
    Datos = Cursor.fetchall()
    #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
    if len(Datos) != 0:
...
def Coger_codigos_ATC_farmaco_especifico(Farmaco):
    ...
    #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
    if len(Datos) != 0: 
        #Esta condición únicamente sirve para lograr una visualización más nítida de los datos.
        #En caso de haber más de un identificador ATC, se devuelve como lista usando un bucle for.
        if len(Datos) == 1:
...
def Farmaco_trata_enfermedades():
    ...
    #Aquí se comprueba que el usuario haya introducido una cadena de texto antes de pulsar "Enter".
    if len(Farmaco) != 0:
        print("\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        Farmaco = Busqueda_laxa_farmacos(Farmaco)
        #Se comprueba que el nombre del fármaco existe en la base de datos.
...
def Obtener_identificadores_farmaco():
    ...
    if len(Farmaco) != 0:
        ...
        if Farmaco != None:
            ...
            #Si se ha encontrado resultado (lista no vacía), se muestra al usuario.
...
def Coger_farmacos_para_enfermedad(): 
    Enfermedad = input("Por favor, indique el nombre de la enfermedad de la que quiere consultar los fármacos involucrados en su tratamiento: ").replace("\t", "")
    #Aquí se comprueba que el usuario haya introducido una cadena de texto antes de pulsar "Enter".
    if len(Enfermedad) != 0:
        print("\n##############################################################################################")
        Enfermedad = Busqueda_laxa_enfermedades(Enfermedad)
        #Se comprueba que el nombre de la enfermedad existe en la base de datos.
        if Enfermedad != None:
            Consulta_SQL = """
                           ...
                           """
            Cursor.execute(Consulta_SQL, (Enfermedad,))
            Datos=Cursor.fetchall()
            #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
            if len(Datos) != 0:
                Encabezado = f"**\tFármacos para el tratamiento de la enfermedad {Enfermedad.upper()}\t**\nN.º\tIdentificador\tNombre\n"
                print(Encabezado)
                Numero_inicial = 1
                #El uso del bucle permite la impresión limpia en pantalla de los resultados.
                for Fila in Datos:
                ...
                print("Esta enfermedad no cuenta con fármacos para tratarla en la base de datos.")
        print("##############################################################################################\n")
    else:
        print("\nNo ha introducido texto. Por favor, inténtelo de nuevo.\n")
...
def Buscar_termino_clave():
    ...
    if len(Termino_clave) != 0:
        ...
        #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
        if len(Datos)!=0:
            Encabezado = f"Enfermedades encontradas para el patrón {Termino_clave.upper()}. Se proporciona el nombre de la enfermedad y su código en cierto vocabulario:\n"
            print(Encabezado)
            #El uso del bucle permite la impresión limpia en pantalla de los resultados.
            for Fila in Datos:
...
def Coger_indicaciones_fenotipo(Farmaco): 
    ...
    #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
    if len(Datos) != 0: 
        Encabezado=f"**\tIndicaciones del fármaco {Farmaco.upper()}\t**\nN.º\tID del fenotipo\tIndicado para\n"
        print(Encabezado)
        Numero_inicial = 1
        #El uso del bucle permite la impresión limpia en pantalla de los resultados.
        for Fila in Datos:
...
def Coger_efectos_secundarios_fenotipo(Farmaco): 
    ...
    #Si se han encontrado resultados (lista no vacía), se muestran al usuario.
    if len(Datos) != 0: 
        Encabezado = f"**\tEfectos secundarios del fármaco {Farmaco.upper()}\t**\nN.º\tID del fenotipo\tEfecto secundario\n"
        print(Encabezado)
        Numero_inicial = 1
        #El uso del bucle permite la impresión limpia en pantalla de los resultados.
        for Fila in Datos: 
...
def Farmacos_provocan_efecto_secundario():
    ...
    #Con esta condición, aseguramos que el usuario no haya pulsado "Enter" sin antes haber escrito algo.
    if len(Efecto) != 0:
        ...
        Datos = Cursor.fetchall()
        print("\n♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣♣")
        #Si se han encontrado resultados (lista no vacía), se ejecuta la consulta más elaborada.
        if len(Datos) != 0: 
            Consulta2_SQL = """
            ...
            Datos = Cursor.fetchall()
            Encabezado = f"**\tFármacos asociados con el efecto secundario {Efecto.upper()}\t**\nN.º\tIdentificador\tFármaco\n"
            print(Encabezado)
            Numero_inicial = 1
            #El uso del bucle permite la impresión limpia en pantalla de los resultados.
            for Fila in Datos:
...
def Coger_primeras_dianas(): 
    Diana=input("Por favor, introduzca el tipo de diana de la que quiere obtener sus 20 primeras instancias: ").replace("\t", "")
    #Aquí se comprueba que el usuario no ha introducido una cadena de texto vacía.
    if len(Diana) != 0:
        Consulta_SQL = """
        ...
        if len(Datos) != 0: 
            ...
            print("No se ha encontrado ninguna diana para el tipo proporcionado.")
        print("*******************************************************************************************\n")
    else:
        print("\nNo ha introducido texto. Por favor, inténtelo de nuevo.\n")
...
def Coger_organismo_mas_dianas(): 
    ...
    #Se da al usuario la posibilidad de ver las dianas asociadas al organismo en cuestión.
    Respuesta_usuario = input("\n¿Quiere consultar el total de las dianas para el organismo seleccionado? [En caso afirmativo, escriba \"Sí\"]: ").replace("\t", "")
    #Si la respuesta es afirmativa, se muestran las dianas asociadas.
    if re.search("[Ss][IiÍí]", Respuesta_usuario):    
...
def Dianas_organismo_concreto():
    ...
    #Esta condición se usa para impedir que el usuario no haya metido texto.
    if len(Organismo) != 0:
        Sentencia="""
...
def Obtener_tipos_diana():
    ...
    Consulta_SQL = """
                   SELECT target_type FROM target GROUP BY target_type
...
#Apartado 6: Borrado
def Borrado():
    print("\nPor medio de esta funcionalidad, se le van a mostrar las diez asociaciones entre fármaco y enfermedad con un score inferido más bajo.")
    ...
    sql_query = """ 
                ...
                ORDER BY inferred_score ASC, drug_name ASC, name ASC
                LIMIT 10;
                """
    ...
    if re.search("[Ss][IiÍí]", Respuesta_usuario):
        ...
        try:
            ...
            if  re.search("[Ss][IiÍí]", Respuesta_usuario3):
                ...
                print("\nUsted ha abandonado la funcionalidad de borrado.")
        except (ValueError, IndexError):
        ...
        #sean incorrectas. 
            ...
            if len(Datos) != 0:
                ...
                if re.search("[Ss][IiÍí]", Respuesta2_usuario): 
                    ...
                    print("\nUsted ha abandonado la funcionalidad de inserciones.")
            ...
            #similar, se hace saber al usuario que debe intentarlo de nuevo. 
                ...
                print(f"\nLo sentimos, este fármaco no está dentro de la base de datos. Por favor, vuelva a intentarlo de nuevo.")
...
def Comprobar_codigo_farmaco(ID_Farmaco, Vocabulario_farmaco): 
    Codigo_farmaco = input("Por favor, introduzca el código identificador del fármaco: ").replace("\t", "")
    if len(Codigo_farmaco) != 0: 
...
def Modificaciones(): 
    ...
    try: 
        ...
        Consulta_SQL = """
                       SELECT count(score) FROM drug_phenotype_effect
                       WHERE phenotype_type = "SIDE EFFECT" AND score < %s AND score != 0;
                       """
        Cursor.execute(Consulta_SQL, (Respuesta_usuario,))
        Datos = Cursor.fetchall()
        if Datos[0][0] > 0: 
            print(f"\nSe ha encontrado un total de {Datos[0][0]} de asociaciones con un valor menor al especificado.")
            Respuesta2_usuario=input(f"¿Está seguro que quiere hacer la modificación? [En caso afirmativo, escriba \"Sí\"]: ").replace("\t", "")
            if re.search("[Ss][IiÍí]", Respuesta2_usuario):
                Modificacion_SQL = """
                ...
                print("\nUsted ha abandonado la funcionalidad de modificaciones.")
        else: 
            print("\nNo hay asocaciones fármaco-efecto secundario con un score menor al introducido.")
    except (ValueError):
                print("\nNecesita meter un número para avanzar. Por favor, inténtalo de nuevo.")
...
def Funcionalidades_4(Ir_a):
    #Se han incluido ciertas consultas intermedias para ahorrar esfuerzo computacional.
    #Si el usuario ha introducido una cadena de texto que no va a dar respuesta, estas
    #consultas permitirán identificarlo a tiempo, impidiendo que se hagan las consultas
    #más complejas.
    if Ir_a == 1:
...
#Llamamos a la función que imprime el menú principal, y guardamos la elección del usuario en "Ir_a". Hay que recordar que las funciones
 #de los diferentes menús, además de imprimir el menú en cuestión, piden al usuario el menú al que quiere dirigirse (apoyándose en la
 #función "Seleccion()".
Ir_a = Menu_principal()
...
def Opciones(Ir_a):
    ...
    #Si es "X" o "x", salimos del programa (gracias a "return").
    elif Ir_a == "x" or Ir_a == "X":
       Mensaje_despedida()
    return 
...
#Cerramos tanto la conexión con la base de datos como el cursor tras al finalizar el programa.
Base.close()
Cursor.close()